...
這樣就取出來了，但為什麼會是 0 呢？ `0`就代表 `Rogers`在這個列表中，從左到右開始的第一個位置，但在電腦存數據時，不是從1開始算的，是從0開始算起的，因為二進制使用的第一個狀態就是 0，所以從 0 開始算的，因此，我們可以透過位置來取得相對應的值。
...
這樣就取到，有發現嗎？如果要取最後一個數值，其實 `省略不打`就可以取到了，那如果要正取 0 到 3 ，同樣的也是可以省略 0 ，代碼寫法如下
